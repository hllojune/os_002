...
본 보고서는 C언어로 작성된 'Basic Interpreter'의 소스 코드를 분석하는 것을 목표로 합니다. 과제 요구사항에 따라, 소스 코드 전체를 라인 단위로 분석하고, 입력 파일(input1.spl)이 처리되어 최종 결과를 출력하기까지의 모든 과정을 코드 레벨에서 추적합니다. 
